...
Данные для списка доступны в формате JSON в файле [data/etsy.json](./data/etsy.json). 
...
Для отображения списка создать компонент `Listing`, который принимает следующие атрибуты:
...
Стили и пример разметки вы можете найти в папке [markup](./markup).
